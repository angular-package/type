...
/**
 * Checks if the `value` is not the type obtained from its object class equal to `'string'`, not a `string` type and not an instance of
 * `String`.
 * @var Type A generic type variable `Type` indicates the captured type of the given `value` via the return type
 * `value is Never<AnyString, Type>` and the `value` parameter of the provided `callback` function `ResultCallback` type.
 ...
 * supplied `callback` function and `payload` optional parameter of the `isNotString()` function from which it captures its value.
