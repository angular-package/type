 ...
 * @param value A rest parameter of `any` type to check its elements against a `boolean` type or an instance of `Boolean` equal to `false`.
 * @returns The return value is an `object` with `every()`, `some()` and `forEach()` as a methods of checking supplied `values`.
 * @angularpackage
 */
export const areFalse = (...values: any[]) => {
  return {
    /**
     ...
     * @returns The return value is a `boolean` indicating whether the provided values of `areFalse()` are a `boolean` type or an instance
     * of `Boolean` equal to `false`.
     */
     ...
     * them before the `result` return. By default, it uses `resultCallback()` function.
     * @param payload An optional `object` of `CallbackPayload` that is assigned to the `payload` of the supplied `callback` function.
     * @returns The return value is a `boolean` indicating whether some of the provided `values` of `areFalse()` are a `boolean` type or
     * an instance of `Boolean` equal to `false`.
     */
