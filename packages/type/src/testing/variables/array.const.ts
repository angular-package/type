...
// Array with `boolean`.
export const ARRAY_BOOLEAN: Array<boolean> = [true, false, TRUE_INSTANCE, FALSE_INSTANCE, false, FALSE, TRUE];
...
// Array with `number`.
export const ARRAY_NUMBER: Array<number> = [1, 2, 3, 4, 7, 10, 1000, 3000, 151233, 12352223, NUMBER, NUMBER_INSTANCE, NUMBER_NEW_INSTANCE];
// Array with 'object'.
export const ARRAY_OBJECT_ONE: Array<ObjectOne> = [{ x: 3000 }, { x: 1 }, { x: 55 }];
export const ARRAY_OBJECT_TWO: Array<ObjectTwo> = [{ x: 'One Two Three', y: 10000  }, { x: 'One Two Three', y: 10000  }, { x: 'One Two Three', y: 10000  }];
// Array with `string`.
export const ARRAY_STRING: Array<string> = [STRING, STRING_INSTANCE, STRING_NEW_INSTANCE, '!@#$%^&*()abcdefghijklmnoprstuwyz'];
// Array with `symbol`.
